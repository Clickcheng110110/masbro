...
import { swiperData, toastSuccessConfig, toastWarningConfig } from "../NewHome";
...
function BuyModalContent({ data, onClose }: IModalContentProps<any>) {
  ...
  const { config, handleSignFunction, balanceStatus } = useConfigContext();
  ...
  const buy = debounce(async () => {
    try {
      ...
      const isCanBuy = await nftMarketContract?.isBuy();
      if (!isCanBuy) {
        toast({
          ...toastWarningConfig,
          title: "未到开市时间，请之后重试",
