...
import { manageAuth } from "@/apis/v2";
...
function Index<T = any>({ isTp }: HeaderProps) {
  ...
  const { data: isManageAuth } = useQuery(
    ["manageAuth", address],
    async ({ queryKey }) => {
      if (!queryKey?.[1]) return;
      const res = await manageAuth(queryKey?.[1]);
      return res?.data;
    },
    {
      enabled: !!address,
  ...
  }, [router?.query?.inviteAddress, userbindsStatus.data]);
  ...
  }, [address, userBindContact?.address, userbindsStatus.data]);
